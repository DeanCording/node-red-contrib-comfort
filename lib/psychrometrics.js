/**
 *    This software is Copyright (c) 2014 Center for the Built Environment
 *
 *    Portions of this software are:
 *    Copyright (c) 1995-2013, Regents of the University of California
 *    All rights reserved.
 *
...
psy.tdb_twb = function (tdb, twb) {
  ...
  a.wetbulb = twb;
  a.w =
    ((PROP.Hfg + (PROP.CpVap - PROP.CpWat) * twb) * wstar -
      PROP.CpAir * (tdb - twb)) /
...
psy.wetbulb = function (tdb, w) {
  ...
  fn = function (t) {
    ...
    newW =
      ((PROP.Hfg - PROP.CpWat - PROP.CpVap * t) * wStar -
        PROP.CpAir * (tdb - t)) /
...
psy.satpress = function (tdb) {
  ...
  if (tKel < 273.15) {
    pascals = Math.exp(
      C1 / tKel +
        C2 +
        tKel * (C3 + tKel * (C4 + tKel * (C5 + C6 * tKel))) +
        C7 * Math.log(tKel)
    );
  } else if (tKel >= 273.15) {
    pascals = Math.exp(
